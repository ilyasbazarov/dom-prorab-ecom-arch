...
# Признак правила-останова проверяется НЕГАТИВНЫМ прогоном (К-34): кейсы ниже — по
# ветке останова, положительный кейс один и удостоверяет только отсутствие ложного
# срабатывания. Гоняются в обеих локалях по тому же мотиву, что и кейсы выше (К-35).
...
EOF
  ...
  ac_case fail "телеметрия вместо смысла (К-42 §4)" <<'EOF'
...
EOF
  ...
  ac_case fail "плотность кодов в одной строке (К-42 §6)" <<'EOF'
