...
**Версия:** v1.44 · **updated:** 2026-08-12 · session `reviewer_f4_13`, блок 2 (Reviewer, Claude Code, Fable): РЕШЕНИЕ OWNER дословно «Р-1 вариант А, апрув пачкой всех 32» — требование корректирующей записи по четырём FIX ревью `ADR-073` снято по образцу ADR-014 (находки утрачены с чатом), апрув всех 32 одобрен. Смену строк статуса, запись Architect по варианту А (с цитатой; по К-40 выйдет `accepted`) и правки находок №1, №3-№6 делает применяющая Architect-сессия: Reviewer патчей не выдаёт. Блоком 1: пересчёт по ADR-089 §1 — записей `proposed` 32, а не 33 (`ADR-069` стоит `accepted`, к 32 её добавил счёт, приписавший ей строки статуса списочных `ADR-070`/`ADR-071`); находок 9 (STOP 1 · FIX 5 · NOTE 3). Попутно сверкой предмета: индекс↔лог разошлись по `ADR-058` (`superseded` против `accepted`; правка строки статуса разрешена ADR-016). Таблица находок и вердикты по всем 32 — `sessions/2026-08-12_reviewer_f4_13.md`. Раньше: пуск собран блоком 9; `F4-17` ждёт воронки, гейта `OQ-08` на ней нет, `CQ-36`…`CQ-44` в SENT; тема правила выхода STATE закрыта. Подробности — §Подробности для модели.
...
**Прошлый шаг.** `reviewer_f4_13`: хвост журнала пересчитан — 32 записи `proposed`, находок 9.
До неё `arch_f4_17_brief`: `F4-17` сужена (ADR-136), ADR-137 и ADR-138 обе `accepted`.

**Следующий шаг.** Строка 13 руками Owner — 14 стадий воронки и снимок в `reference/`; сразу за
ней прогон `F4-17`. По `F4-13` — Architect-сессия применения: запись по варианту А, 32 строки
статуса, находки №1 и №3-№6; решение Owner получено, очередь за применением.
...
сюда оно не переписывается. Строк 68, из них с собственным файлом сессии 45; остальные держит
