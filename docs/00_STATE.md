...
**Версия:** v1.40 · **updated:** 2026-08-12 · session `arch_f4_17_brief`, блок 7 (Architect режим A, Claude Code, Opus): АПРУВ ADR-138 — обе записи про STATE в `accepted`, тема закрыта целиком. Итог темы: у каждого раздела `docs/00_STATE.md` есть правило выхода и машинный лимит, файл 268 → 21 КБ без потери содержания, а сама норма въехала в процессные документы — К-19 и К-23 переизданы, форма session-блока требует строку индекса вместо прозы, карта репо и строка `M1-10` догнаны, сообщения хука зовут в действующие места. Обратная деградация закрыта хуком, а не текстом: три лимита срабатывают на коммите. Раньше: `F4-17` сужена до восьми шагов внутри CRM, её единственный гейт — заведённая воронка (строка 13), остаток в `F4-24`; гейта OQ-08 на задаче нет, `CQ-36`…`CQ-44` в SENT. Подробности — §Подробности для модели.
