...
**Версия:** v1.39 · **updated:** 2026-08-12 · session `arch_f4_17_brief`, блок 6 (Architect режим A, Claude Code, Opus): АПРУВ ADR-137 (в `accepted`) И ЕЁ ДОСТРОЙКА ДО КОНЦА (ADR-138, `proposed` по К-40). Сплошной поиск по К-27 нашёл четыре живых места, продолжавших предписывать снятое поведение; главное — К-23 велела заводить аномалию ровно в тот раздел, который ADR-137 ограничила пятнадцатью строками. К-23 переиздана: носитель аномалии — реестр открытых вопросов либо `docs/10_SCARS.md`. Форма session-блока приведена к индексу, карта репо и строка `M1-10` догнаны, сообщения хука зовут в действующие места. Совпадений 53 и 91, класс истории остаётся, живых мест исправлено восемь; в промтах, CLAUDE.md и ранбуке предписаний писать в снятые места 0. Раньше в блоке 5: STATE ужат 268 → 21 КБ правилом выхода для каждого раздела. Ещё раньше: `F4-17` сужена до восьми шагов, гейт — воронка, остаток в `F4-24`; гейта OQ-08 нет, `CQ-36`…`CQ-44` в SENT. Подробности — §Подробности для модели.
...
21 КБ правилом выхода (ADR-137, `accepted`), и правило въехало в процессные документы (ADR-138).
